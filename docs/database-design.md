...
The Digital Stamp Card MVP uses MySQL UUID string primary keys and stores only hashes of authentication and QR tokens.
...
erDiagram
    ...
    CUSTOMERS ||--o{ CUSTOMER_QR_TOKENS : generates
    STAFF ||--o{ CUSTOMER_QR_TOKENS : uses
    CUSTOMERS ||--o{ STAMP_TRANSACTIONS : receives
    STAFF ||--o{ STAMP_TRANSACTIONS : performs
    REWARDS ||--o{ STAMP_TRANSACTIONS : redeemed_in
    CUSTOMER_QR_TOKENS ||--o| STAMP_TRANSACTIONS : creates
```
...
| `customer_qr_tokens` | One-time, expiring customer identification tokens. | Generated by a customer and optionally used by one staff member. |
...
- A used QR must identify when and by which staff member it was used.
