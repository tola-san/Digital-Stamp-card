    ...
    <main className="min-h-screen bg-[#f4f7f9] px-4 py-5 text-slate-950 sm:px-6 sm:py-8">
      <div className="mx-auto max-w-lg">
        <header className="mb-5 flex items-center justify-between">
          ...
          <span className="flex size-11 items-center justify-center rounded-xl bg-emerald-700 text-white shadow-sm"><Gift className="size-5" /></span>
        </header>

        <section className="overflow-hidden rounded-3xl bg-white shadow-[0_24px_70px_-34px_oklch(0.25_0.06_165/0.35)] ring-1 ring-slate-900/6">
          ...
          {customer && card && <div className="p-6 sm:p-7">
            <div className="text-center"><span className={`mx-auto flex size-14 items-center justify-center rounded-2xl ${confirmation ? "bg-emerald-100 text-emerald-700" : "bg-sky-100 text-sky-700"}`}>{confirmation ? <BadgeCheck className="size-7" /> : <UserRound className="size-6" />}</span><h1 className="mt-4 text-xl font-semibold">{confirmation ? "បានបន្ថែមត្រាជោគជ័យ" : "ពិនិត្យព័ត៌មានអតិថិជន"}</h1><p className="mt-1 text-sm text-slate-500">{customer.name} · {customer.phone}</p></div>
            <div className="mt-6 rounded-2xl bg-slate-50 p-5 ring-1 ring-slate-900/5"><div className="flex items-end justify-between"><div><p className="text-sm text-slate-500">ត្រាបច្ចុប្បន្ន</p><p className="mt-1 text-3xl font-semibold tabular-nums">{card.stamp_count}<span className="text-base text-slate-400"> / {card.required_stamps}</span></p></div><span className="text-sm font-semibold text-emerald-700">{stampProgress(card)}%</span></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-emerald-600 transition-[width] duration-500" style={{ width: `${stampProgress(card)}%` }} /></div><p className="mt-3 flex items-center gap-2 text-sm text-slate-500"><Sparkles className="size-4 text-amber-500" />នៅសល់ {Math.max(0, card.required_stamps - card.stamp_count)} ត្រាទៀត</p></div>
