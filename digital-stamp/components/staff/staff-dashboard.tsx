            ...
            <section className="mt-6 grid items-start gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(340px,.9fr)]" id="scan">
              <div className="overflow-hidden rounded-3xl bg-white shadow-[0_24px_70px_-36px_oklch(0.25_0.05_160/0.3)] ring-1 ring-slate-900/6">
                ...
                <div className="p-6 sm:p-7">
                  ...
                  <form className="mt-5" onSubmit={handlePreview}><label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="qr-token">លេខកូដ QR</label><div className="flex flex-col gap-3 sm:flex-row"><input autoComplete="off" className="h-12 min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-4 text-sm outline-none transition-[border-color,box-shadow] duration-150 placeholder:text-slate-400 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-600/12" id="qr-token" onChange={(event) => { setToken(event.target.value); setError(""); }} placeholder="បញ្ចូល token ពី QR…" value={token} /><Button className="h-12 rounded-xl bg-emerald-700 px-5 font-semibold text-white shadow-sm transition-[background-color,transform] duration-150 hover:bg-emerald-800 active:scale-[0.96]" disabled={isScanning} type="submit">{isScanning ? <LoaderCircle className="animate-spin" /> : <ScanLine />}ពិនិត្យកូដ</Button></div></form>
